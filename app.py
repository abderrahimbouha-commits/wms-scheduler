...
with tab1:
    ...
    if uploaded_file1 and st.button("Generate Smoothing", key="btn1"):
        ...
        # ... (Your existing Smoothing logic here) ...
        st.success("Smoothing complete!")

# --- TAB 2: NEW LEVELING TOOL ---
with tab2:
    ...
    if uploaded_file2 and st.button("Calculate Leveling", key="btn2"):
        ...
        # Validation
        if not all(col in df.columns for col in ['OT', 'Equipment', 'duree', 'MH', 'Section']):
            st.error("Missing columns: OT, Equipment, duree, MH, Section")
        else:
            # Sort by Duree (Longest first) for better smoothing
            ...
            for _, row in df.iterrows():
                ...
                current_time = end # Update for next task
