...
if check_password():
    ...
    with tabs[3]:
        ...
        # USE UPLOADER TO FIX FILE PATH ISSUES
        inspection_file = st.file_uploader("Upload 'Convoyeur.csv'", type=['csv'])
        
        if inspection_file:
            try:
                # header=2 skips the first 2 blank rows
                df = pd.read_csv(inspection_file, header=2)
                # Drop column 0 (which contains the leading comma/empty data)
                df = df.iloc[:, 1:]
                ...
                if selected:
                    ...
                    st.write(f"Total Path: {route_df['length_m'].sum():.2f}m")
